...
describe('Http-Logs Service (unit)', () => {
    ...
    it('test case throw middleware', () => {
        ...
        let httpError = new HttpError('Custom Error', HttpStatusCodes.BAD_REQUEST, HttpStatus.BAD_REQUEST);

        // test case when error is undefined
        HttpErrors.Throw(undefined as unknown as HttpError, mockRequest, mockResponse as unknown as Response, nextFunction);
        sinon.assert.calledWith(mockResponse.status, HttpStatusCodes.INTERNAL_SERVER_ERROR);
        ...
        HttpErrors.Throw(httpError, mockRequest, mockResponse as unknown as Response, nextFunction);
        ...
        httpError = new HttpError('Custom Error', HttpStatusCodes.SERVICE_UNAVAILABLE, HttpStatus.SERVICE_UNAVAILABLE);
        HttpErrors.Throw(httpError, mockRequest, mockResponse as unknown as Response, nextFunction);
        sinon.assert.calledWith(mockResponse.status, HttpStatusCodes.SERVICE_UNAVAILABLE);
        sinon.assert.calledWithMatch(mockResponse.send, {
        ...
        httpError = new HttpError('NotFound', HttpStatusCodes.NOT_FOUND, HttpStatus.NOT_FOUND);
        HttpErrors.Throw(httpError, mockRequest, mockResponse as unknown as Response, nextFunction);
    ...
    it('test case http-error using default error', () => {
        ...
        // Default status code
        expect(httpError.statusCode).to.equal(HttpStatusCodes.INTERNAL_SERVER_ERROR);
        // Default error code

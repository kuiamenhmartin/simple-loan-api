...
describe('RouterTryCatch (unit)', () => {
    ...
    it('should test router try catch middleware', async () => {        
        ...
        await RouterTryCatch(mockController)(mockRequest as AnyData, mockResponse as AnyData, nextFunction);
        ...
        mockController = async () => Promise.reject(HttpStatusCodes.INTERNAL_SERVER_ERROR);
        // test case when controller response a success
        await RouterTryCatch(mockController)(mockRequest as AnyData, mockResponse as AnyData, nextFunction);
        sinon.assert.calledWith(nextFunction, HttpStatusCodes.INTERNAL_SERVER_ERROR);
        ...
        mockController = async () => Promise.resolve({
            status: undefined
        } as AnyData);

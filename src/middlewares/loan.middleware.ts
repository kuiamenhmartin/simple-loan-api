...
import { CAR_LOAN_AMOUNT_LIMIT, PERSONAL_LOAN_AMOUNT_LIMIT } from "../constants/loan.constants";
 ...
 * @returns Promise<boolean>
 */
export const isLoanAmountExceedsLimit = async (loanAmount: number, meta: Meta) => {
    const loanType = (meta.req.body as LoanApplication).loanType
    if (loanType === LoanType.CAR && loanAmount > CAR_LOAN_AMOUNT_LIMIT) {
        throw new Error(`${meta.path} should not exceed ${LoanType.CAR} loan amount limit of ${CAR_LOAN_AMOUNT_LIMIT}`);
    }

    if (loanType === LoanType.PERSONAL && loanAmount > PERSONAL_LOAN_AMOUNT_LIMIT) {
        throw new Error(`${meta.path} should not exceed ${LoanType.PERSONAL} loan amount limit of ${PERSONAL_LOAN_AMOUNT_LIMIT}`);
    }
...
export const LoanBodyValidation: ValidationChain[] = [
    ...
    body('loanType')
        ...
        .withMessage(`loanType must be one of the following ${Object.values(LoanType).join(' | ')}`)
];
...
export const LoanParamValidation: ValidationChain[] = [
    ...
    .withMessage('Loan Application Id must be a valid hexadecimal of length 32'),
];
